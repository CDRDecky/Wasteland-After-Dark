...
/datum/disease/curling_thirteen/stage_act()
	...
	switch(stage)
		...
		if(2)
			if(prob(2))
				to_chat(affected_mob, "<span class='warning'>You scratch at an itch.")
				affected_mob.adjustBruteLoss(3, FALSE)
			if(prob(2))
				affected_mob.emote("cough")
				affected_mob.adjustOxyLoss(2, FALSE)
		...
		if(4) // That's the part where you start dying
			...
			if(prob(3))
				...
				affected_mob.adjustBruteLoss(10, FALSE)
